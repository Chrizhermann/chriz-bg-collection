...
function candidateSelect(labelText: string, candidates: readonly GameCandidate[], selectedId: string): { wrapper: HTMLElement; select: HTMLSelectElement; finding: HTMLElement } {
  ...
  wrapper.append(label, select, path, finding);
  const refresh = (): void => {
...
export function gamesScreen(
  ...
  const bg1 = candidateSelect("Baldur's Gate source", discovery.bg1Candidates, selectedBg1Id);
  const bg2 = candidateSelect("Baldur's Gate II source", discovery.bg2Candidates, selectedBg2Id);
